function getIpInfo() {
  fetch(
    'https://restapi.amap.com/v3/ip?key=e2b04289e870b005374ee030148d64fd&s=rsv3'
  )
    .then((res) => res.json())
    .then((data) => {
      fetch(
            ...
            function updateTime() {
              var cd = new Date(),
                ...
                card_clock_date =
                  ...
                  ' ' +
                  week[cd.getDay()],
                card_clock_dackorlight = cd.getHours(),
                card_clock_dackorlight_str
              if (
                ...
                var card_clock_time_dom =
                    document.getElementById('card-clock-time'),
                  card_clock_date_dom = document.getElementById(
                    'card-clock-clockdate'
                  ),
                  card_clock_dackorlight_dom = document.getElementById(
                    'card-clock-dackorlight'
                  )
                ;(card_clock_time_dom.innerHTML = card_clock_time),
                  (card_clock_date_dom.innerHTML = card_clock_date),
                  (card_clock_dackorlight_dom.innerHTML =
                    card_clock_dackorlight_str)
              }
            }
            function zeroPadding(num, digit) {
              for (var zero = '', i = 0; i < digit; i++) zero += '0'
